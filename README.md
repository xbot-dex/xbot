...
### Запуск:
*Windows (только 10-я 64-битная)*: Скачиваем **[xBot-USDT.exe](https://github.com/xbot-dex/xbot/releases/latest)** и запускаете программу  
*Linux (минимум 18.04)*: Скачиваем **[xBot-USDT](https://github.com/xbot-dex/xbot/releases/latest)** (  `wget https://github.com/xbot-dex/xbot/releases/download/{ВЕРСИЯ}/{НАЗВАНИЕ_БОТА}`,  где вместо ***{ВЕРСИЯ}*** указываете последнюю версию, а вместо ***{НАЗВАНИЕ_БОТА}*** xBot-BTC или xBot-USDT),  
прописываете ему права командой `chmod 755 xBot-USDT`,  
после чего запускаете командой `./xBot-USDT`
...
*Linux*: В директории с ботом вводите команду на удаление старой версии `rm xBot-USDT`,  
затем вводите команду `wget https://github.com/xbot-dex/xbot/releases/download/{VERSION}/xBot-USDT`,  
где вместо ***{VERSION}*** указываете последнюю версию, после чего даёте права файлу `chmod 755 ./xBot-USDT`  
и запускаете его командой `./xBot-USDT`

### Работа с screen:
1) При первом запуске/перезагрузке сервера вводим `screen -S <name_screen>` (например `screen -S xbot`), попадаем сразу в новый screen и запускаем `./xBot-BTC` или `./xBot-USDT`  
