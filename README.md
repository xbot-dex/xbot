...
### Запуск:
*Windows (только 10-я 64-битная)*: Скачиваем **[X-Bot-USDT.exe](https://github.com/xbot-dex/xbot/releases/latest)** и запускаете программу  
*Linux (минимум 18.04)*: Скачиваем **[X-Bot-USDT](https://github.com/xbot-dex/xbot/releases/latest)** (  `wget https://github.com/xbot-dex/xbot/releases/download/{ВЕРСИЯ}/{НАЗВАНИЕ_БОТА}`,  где вместо ***{ВЕРСИЯ}*** указываете последнюю версию, а вместо ***{НАЗВАНИЕ_БОТА}*** X-Bot-BTC или X-Bot-USDT),  
прописываете ему права командой `chmod 755 X-Bot-USDT`,  
после чего запускаете командой `./X-Bot-USDT`
...
*Linux*: В директории с ботом вводите команду на удаление старой версии `rm X-Bot-USDT`,  
затем вводите команду `wget https://github.com/xbot-dex/xbot/releases/download/{VERSION}/X-Bot-USDT`,  
где вместо ***{VERSION}*** указываете последнюю версию, после чего даёте права файлу `chmod 755 ./X-Bot-USDT`  
и запускаете его командой `./X-Bot-USDT`

### Работа с screen:
1) При первом запуске/перезагрузке сервера вводим `screen -S <name_screen>` (например `screen -S xbot`), попадаем сразу в новый screen и запускаем `./X-Bot-USDT` или `./X-Bot-USDT`  
...
Также, при переносе бота в другую директорию, не забудьте перенести файл xbot.db вместе с ним.
...
2. Вводим любое имя бота (например test_push_xbot)  
3. Далее придумываем уникальный юзернейм для бота, который оканчивается на 'bot' (например ***exambot_xbot***)  
...
5. Создайте публичный или частный канал (Меню -> Создать канал), придумайте его название. Далее, если канал будет публичным, придумайте ему @name (например ***@test_push_xbot_channel*** (@name — это название в ссылке t.me/test_push_xbot_channel)), добавьте в канал созданного ранее бота ***exambot_xbot*** и сделайте его администратором с возможностью изменения профиля канала и публикацией сообщений (первые две галочки возможности администратора). Если же канал будет приватным, то Вам нужно будет получить его chat_id (добавьте Telegram-бота ***exambot_xbot*** в частный канал и отправьте там любое сообщение, после чего перейдите в браузере по ссылке `https://api.telegram.org/bot{YOUR_API_KEY}/getUpdates`, где вместо ***{YOUR_API_KEY}*** введите Telegram token, выданный BotFather'ом. Затем среди массива данных найдите ключ "chat" и его значение "id". Оно будет выглядеть примерно как "-1XXXХХХХХХХХХ")  
6. Запустите xBot и в настройках параметров API и Telegram включите Telegram-уведомления, затем введите API token, который выдал Вам BotFather и @name/chat_id Вашего канала, где в администраторах находится Telegram-Bot, созданный на 5 этапе (***@test_push_xbot_channel***)  
Теперь каждый раз, когда бот полностью закрывает позицию на продажу, Вы будете получать уведомление в Ваш канал!

![](./screen/img-3.png)  
![](./screen/img-4.png)  

### Торговые параметры (на примере USDT):
* `min_balance` [min >= 0 USDT] — Оставлять свободный баланс? Если значение больше 0, то бот будет накапливать свободный депозит в BTC до достижения этой суммы, не совершая новых сделок на покупку/усреднение активов  
* `min_order` [min >= 10 USDT] — Размер первого ордера на покупку в BTC, с которого бот начинает работать по торговой паре (например 0.0001)  
